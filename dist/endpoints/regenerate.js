...
const statusGeneration = new Map();
...
function invalidateStatusMemo(slug) {
...
export const createRegenerateHandler = (resolvedConfig)=>{
    const handler = async (req)=>{
        ...
        const CHUNK_SIZE = 25;
        const queueChunk = async (ids)=>{
            await Promise.all(ids.map((docId)=>req.payload.jobs.queue({
                ...
                while(Date.now() - startAt < WAVE_BUDGET_MS){
                    ...
                    try {
                        ...
                        req.payload.logger.error({
                            err
                        }, 'Image optimizer: wave run failed');
                        return;
                    }
                }
                req.payload.logger.warn({
                    collectionSlug,
                    budgetMs: WAVE_BUDGET_MS
